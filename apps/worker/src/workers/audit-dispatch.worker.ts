...
@Injectable()
export class AuditDispatchWorker implements OnModuleInit, OnModuleDestroy {
  private worker?: Worker;
  private readonly logger = createLogger("worker.audit");

  onModuleInit() {
    this.worker = new Worker(
      QUEUE_NAMES.auditDispatch,
  ...
  private async handle(job: Job) {
    ...
    this.logger.info("audit dispatch job received", {
      queue: job.name,
